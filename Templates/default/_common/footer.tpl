<aside class="behindTheSite footerSide">    
    <h3>Behind the site</h3>
    <span>
        This site is flavored with 
        <a href="{$_root}/page/projects/beedon">Beedon</a>, 
        <a href="http://php.net" target="blank">PHP</a>, 
        HTML5, CSS3, and a 
        <a href="http://jquery.com" target="blank">jQuery</a>
        based JavaScript awesome. <br/> <br/>
       
        There're lots of tools which helped me to build
        this kitchen. I'm thankful to 
        <a href="http://debian.org" class="debian" target="blank">Debian</a> 
        (the universal operating system),
        <a href="http://www.vim.org/" target="blank">Vim</a> 
        and 
        <a href="http://www.gimp.org/" target="blank">Gimp</a> . <br/><br/>


        FBI Warning!!! <br/>
        This blog can become <em>'schizophrenic'</em> in somehow!

        <span class="validators">
            <a class="html5" href="http://validator.w3.org/check?uri=referer" target="blank">
                Valid HTML5
            </a>
            <a class="css3" href="http://jigsaw.w3.org/css-validator/check/referer?profile=css3" target="blank">
                Valid CSS3
            </a>
        </span> <!-- .validators -->
    </span>
</aside>

<aside class="aboutAuthor footerSide">    
    <h3>About author</h3>
    <span>
        <img src="{$_staticPath}/img/yuxel.jpg" />
        Osman Yuksel, works as a front-end developer in a 
        software house named 
        <a href="http://www.tart.com.tr/" target="blank">Tart</a> 
        with great Bospohourus view in Istanbul. <br/> <br/>

        After 6 years of blogging in Turkish, he decided to write 
        something in English. He's passionate about open source
        web technologies. <br/> <br/>

        He is well known as <em>'The front-end guy who uses Vim'</em>, 
        <em>'web standarts evangelist who even sends email in valid markup'</em>,
        <em>'man in <span class="opera">`browse me`</span> tshirt'</em> and
        <em>'blogger who introduces himself speaking in third person'</em>
        among his entourage. <br/> <br/>

        You can check his <a href="http://yuxel.net/cv/_en" target="blank">CV</a> 
        and 6 years old <a href="http://yuxel.net" target="blank">Turkish blog</a>.

    </span>

</aside>
<div class="clear"></div>

