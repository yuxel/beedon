...
$erfile=file_get_contents("$atemp_dir/msg.html");


$oldnick=htmlspecialchars($_POST['oldnick']);
...
if(strtolower($nick)!=strtolower($oldnick)) {
    if(nick_exists($nick)){
        $errormsg=$errormsg.nick_exists($nick)."<br>";
        $errors++;
    }
}
...
if($errors==0) {
    ...
    @db_query("update ${dbprefix}members set nick='$nick', password='$newpass', theme='$theme', mail='$usermail', language='$lang', mail_on='$mail_on', real_name='$realname', icq='$icq', msn='$msn', yahoo='$yahoo', jabber='$jabber' , signature='$signature' , avatar='$avatar', location='$location', homepage='$homepage',auth='$userauth'  where id='$userid'");

    $bee_name=$GLOBALS['bee_name'];
    if($nick==$bee_name){ // eðer kullanýcý kendi hesabýný düzenlerse
